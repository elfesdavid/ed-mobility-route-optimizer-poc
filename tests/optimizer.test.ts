...
describe("Route optimizer proof of concept", () => {
  ...
  it("G – keeps completed legs immutable during replanning", () => {
    ...
    const active: Mission = { id: "active", driverId: world.driver.id, startTime: world.driver.availableFrom, startLocation: LOCATIONS.DUSSELDORF, optimizationMode: "MAX_REVENUE", riskProfile: "NORMAL", legs: [{ type: "TRAIN", transportMode: "PUBLIC_TRANSPORT", origin: LOCATIONS.DUSSELDORF, destination: LOCATIONS.HAMBURG, departureTime: "2026-09-07T06:00:00.000Z", arrivalTime: "2026-09-07T10:00:00.000Z", revenue: eur(0), cost: eur(2990), distanceKm: 390, durationMinutes: 240, confidence: "HIGH" }, { type: "TRAIN", transportMode: "PUBLIC_TRANSPORT", origin: LOCATIONS.HAMBURG, destination: LOCATIONS.BREMEN, departureTime: "2026-09-07T10:00:00.000Z", arrivalTime: "2026-09-07T11:00:00.000Z", revenue: eur(0), cost: eur(1990), distanceKm: 120, durationMinutes: 60, confidence: "HIGH" }], scoreBreakdown: { totalRevenue: eur(0), estimatedTravelCosts: eur(4980), estimatedOtherCosts: eur(0), estimatedSurplus: eur(-4980), missionDurationMinutes: 300, revenuePerHour: 0, emptyDistanceKm: 510, riskPenalty: 0, softConstraintPenalty: 0, finalScore: -4980 }, explanationItems: [] };
  ...
  it("AB – warns early when a confirmed pickup window has already expired", () => {
    ...
    const active: Mission = { id: "completed-leg", driverId: world.driver.id, startTime: world.driver.availableFrom, startLocation: LOCATIONS.DUSSELDORF, optimizationMode: "MAX_REVENUE", riskProfile: "NORMAL", legs: [{ type: "TRAIN", transportMode: "PUBLIC_TRANSPORT", origin: LOCATIONS.DUSSELDORF, destination: LOCATIONS.HAMBURG, departureTime: "2026-09-07T06:00:00.000Z", arrivalTime: "2026-09-07T10:30:00.000Z", revenue: eur(0), cost: eur(2990), distanceKm: 390, durationMinutes: 270, confidence: "HIGH" }], scoreBreakdown: { totalRevenue: eur(0), estimatedTravelCosts: eur(2990), estimatedOtherCosts: eur(0), estimatedSurplus: eur(-2990), missionDurationMinutes: 270, revenuePerHour: 0, emptyDistanceKm: 390, riskPenalty: 0, softConstraintPenalty: 0, finalScore: -2990 }, explanationItems: [] };
  ...
  it("AC – treats leg departure as in progress and leg arrival as completed", () => {
    const world = createBaseWorld();
    const active: Mission = { id: "boundary", driverId: world.driver.id, startTime: world.driver.availableFrom, startLocation: LOCATIONS.DUSSELDORF, optimizationMode: "MAX_REVENUE", riskProfile: "NORMAL", legs: [{ type: "TRAIN", transportMode: "PUBLIC_TRANSPORT", origin: LOCATIONS.DUSSELDORF, destination: LOCATIONS.HAMBURG, departureTime: "2026-09-07T06:00:00.000Z", arrivalTime: "2026-09-07T10:00:00.000Z", revenue: eur(0), cost: eur(2990), distanceKm: 390, durationMinutes: 240, confidence: "HIGH" }], scoreBreakdown: { totalRevenue: eur(0), estimatedTravelCosts: eur(2990), estimatedOtherCosts: eur(0), estimatedSurplus: eur(-2990), missionDurationMinutes: 240, revenuePerHour: 0, emptyDistanceKm: 390, riskPenalty: 0, softConstraintPenalty: 0, finalScore: -2990 }, explanationItems: [] };
  ...
  it("W – preserves taxi mode when replanning after a completed taxi leg", () => {
    ...
    const active: Mission = { id: "taxi-active", driverId: world.driver.id, startTime: world.driver.availableFrom, startLocation: LOCATIONS.DUSSELDORF, optimizationMode: "MAX_REVENUE", riskProfile: "NORMAL", legs: [{ type: "TAXI", transportMode: "TAXI", origin: LOCATIONS.DUSSELDORF, destination: LOCATIONS.COLOGNE, departureTime: "2026-09-07T06:00:00.000Z", arrivalTime: "2026-09-07T07:00:00.000Z", revenue: eur(0), cost: eur(2500), distanceKm: 40, durationMinutes: 60, confidence: "HIGH" }], scoreBreakdown: { totalRevenue: eur(0), estimatedTravelCosts: eur(2500), estimatedOtherCosts: eur(0), estimatedSurplus: eur(-2500), missionDurationMinutes: 60, revenuePerHour: 0, emptyDistanceKm: 40, riskPenalty: 0, softConstraintPenalty: 0, finalScore: -2500 }, explanationItems: [] };
  ...
  it("AE – preserves own-vehicle mode and vehicle after a completed own-vehicle order", () => {
    const world = createBaseWorld();
    world.driver.currentTransportMode = "OWN_VEHICLE";
    world.driver.currentVehicleId = "vehicle-mercedes-a";
    const completed = opportunity("completed-own-order", "DIRECT_ORDER", "Düsseldorf → Köln", "DUSSELDORF", "COLOGNE", 10000, "2026-09-07T06:00:00.000Z", "2026-09-07T07:00:00.000Z", "2026-09-07T07:00:00.000Z", "2026-09-07T12:00:00.000Z", 10, [cargo("Paket", "TINY", 1)]);
    const followUp = opportunity("own-replan-follow-up", "DIRECT_ORDER", "Köln → Dortmund", "COLOGNE", "DORTMUND", 10000, "2026-09-07T07:00:00.000Z", "2026-09-07T12:00:00.000Z", "2026-09-07T08:00:00.000Z", "2026-09-07T16:00:00.000Z", 10, [cargo("Paket", "TINY", 1)]);
    world.opportunities = [completed, followUp];
    const firstResult = transition(world, initialSearchState(world), completed, new FixtureRoutingProvider(world), world.vehicles[0]);
    expect(firstResult.rejection).toBeUndefined();
    const active: Mission = { id: "own-vehicle-active", driverId: world.driver.id, startTime: world.driver.availableFrom, startLocation: LOCATIONS.DUSSELDORF, optimizationMode: "MAX_REVENUE", riskProfile: "NORMAL", legs: firstResult.state.legs, scoreBreakdown: scoreLegs(firstResult.state.legs, world.driver.availableFrom, { optimizationMode: "MAX_REVENUE", riskProfile: "NORMAL" }, DEFAULT_CONFIG), explanationItems: [] };
    expect(active.legs.at(-1)?.transportMode).toBe("OWN_VEHICLE");
    const result = replan(active, world, "2026-09-07T08:00:00.000Z", { optimizationMode: "MAX_REVENUE", riskProfile: "NORMAL" });
