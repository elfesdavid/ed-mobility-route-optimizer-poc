...
function addWaitIfNeeded(state: SearchState, destination: Location, until: string): SearchState {
  ...
  return {
    ...state,
    currentTime: until,
    legs: [...state.legs, { type: "WAIT", transportMode: state.transportMode, origin: destination, destination, departureTime: state.currentTime, arrivalTime: until, cargoItems: state.carriedCargo.length > 0 ? state.carriedCargo : undefined, revenue: eur(0), cost: eur(0), distanceKm: 0, durationMinutes: waitMinutes, confidence: "HIGH" }],
  };
}
