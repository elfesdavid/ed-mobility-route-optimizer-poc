...
function addWaitIfNeeded(state: SearchState, destination: Location, until: string): SearchState {
  ...
  return {
    ...state,
    currentTime: until,
    legs: [...state.legs, { type: "WAIT", transportMode: state.transportMode, continuationTransportMode: state.transportMode, origin: destination, destination, departureTime: state.currentTime, arrivalTime: until, cargoItems: state.carriedCargo.length > 0 ? state.carriedCargo : undefined, revenue: eur(0), cost: eur(0), distanceKm: 0, durationMinutes: waitMinutes, confidence: "HIGH" }],
  };
}
...
function addRoute(state: SearchState, route: RouteOption, destination: Location, customerVehicle = false, vehicleId?: string): SearchState {
  ...
  return {
    ...
    legs: [...state.legs, { type: routeLegType(route.mode, customerVehicle), transportMode: route.mode, continuationTransportMode: route.mode, origin: state.location, destination, departureTime: route.departureTime, arrivalTime: route.arrivalTime, vehicleId, connectionId: route.connectionId, cargoItems: state.carriedCargo.length > 0 ? state.carriedCargo : undefined, revenue: eur(0), cost: route.cost, distanceKm: route.distanceKm, durationMinutes: route.durationMinutes, confidence }],
