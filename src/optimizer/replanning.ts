...
export function replan(activeMission: Mission, updatedWorld: WorldState, currentTime: string, preferences: MissionPreferences, optimizer = new RouteOptimizer()): ReplanResult {
  ...
  const replanningWorld: WorldState = structuredClone({
    ...
    driver: { ...updatedWorld.driver, currentLocation, currentTransportMode: preservedLegs.at(-1)?.type === "TRAIN" || preservedLegs.at(-1)?.type === "BUS" ? "PUBLIC_TRANSPORT" : "WALKING", currentVehicleId: preservedLegs.at(-1)?.type === "DRIVE" ? preservedLegs.at(-1)?.vehicleId : undefined, availableFrom: currentTime },
  });
